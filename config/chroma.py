...
def get_collection(collection_name: str = "medical_collection") -> chromadb.Collection:
    """Get or create a collection with OpenAI embeddings."""
    ...
    # Safe to call every time - won't duplicate if collection already exists
...
def test_chroma_connection() -> bool:
    """Ping ChromaDB to verify it's alive (used in health check)."""
    try:
        client = get_chroma_client()
